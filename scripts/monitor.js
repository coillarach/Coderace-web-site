...
function zeroMap() {
	var mapOptions = {
		center: new google.maps.LatLng(gameLatitude, gameLongitude),
		zoom: gameZoom,
	...
	if (request.readyState == 4) {
		if (request.status == 200 || request.status == 304) {
				...
				if (markersSet) {
					...
					if (data.locations[m].team == "") {
						markers[m].listenerId = google.maps.event.addListener(markers[m], 'click', function() {
							infowindow.setContent(showClue(this.index));
							infowindow.open(map, markers[this.index]);
						});
						var image = {	url: imageDir + gameId + "/free.svg",
						...
						var image = {
								url: imageDir + data.gameId + "/" + data.locations[m].icon,
			...
			for (var p in data.players) {
				if (markersSet) {
					...
					players[p] = new google.maps.Marker({
						...
						icon: {url: imageDir + data.gameId + "/player.svg"}
  					});
			...
			for (var m in data.events) {
				...
				alertString += "showEvent(" + k + ", " + j + ", \"" + markers[j].clue.replace("'","&apos;") + "\", \"" + data.events[m].message + "\", \"";
...
function showEvent(p, l, clue, message, detail) {
	...
	outString += "<p id='bubble'>" + clue + "<br /><br />";
	outString += players[p].username + " said: " + detail + "<br /><br />";
	outString += message + "</p>";
	outString += "</div>";
	infowindow.setContent(outString);
